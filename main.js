...
d3.csv(csvUrl).then(function(csv) {
    ...
    function stepTransition(year) {
        var transitionOption = d3.select("#selectButton").property("value")
        ...
        console.log(transitionOption)
            // run the updateChart function with this selected option
        //highlight(transitionOption)

        d3.selectAll('circle')
            .data(yearData)
            .transition(1000).delay(50)
            .attr('cy', function(d) {
                const conf= d.Conf
                if (conf === "NBA"){
                    return yScale(77)
    ...
    var showTooltip = function(e,d) {
        ...
        tooltip
        .style("opacity", 1)
        .html(d.Team+"<br>"+"W/L: "+d.Wins+"-"+d.Loss)
    ...
    const leaguerScale = d3.scaleSqrt().domain(leagueExtent).range([15, 65])
    ...
    function highlight(selectTeam) {
        d3.selectAll('circle') 
             .attr('fill', function(d) {
                 const conf = d.Conf
                 const t = d.Team
                  if (selectTeam=="All NBA"){
                     const fill = confColors[conf]
                     return fill
                  }else{
                    if(selectTeam===t) {
